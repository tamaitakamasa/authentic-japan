...
export function PageHeader({ title, description, mv }: PageHeaderProps) {
	...
	return (
		<div className="p-page__header u-full-bleed overflow-y-hidden" ref={containerRef}>
			<div className="p-page__inner c-container">
				{title && <h2 className="p-page__title">{title}</h2>}
				{content && <p className="p-page__description" dangerouslySetInnerHTML={{ __html: content }} />}
			</div>
